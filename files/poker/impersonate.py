...
def main():
	...
	streak = 0
	p = int(sys.argv[2])#getPrime(300)
	q = int(sys.argv[3])#getPrime(300)
	phi = (p-1)*(q-1)
	N = p*q
	...
	while streak < 10:
		...
		print(computer_e, computer_d, phi, N)
		sys.exit()
		...
		for x in range(len(enc_deck)):
			while True:
				try:
					enc_card = enc_deck[x]#int(inputimeout(prompt=f'Card {x+1} >> ', timeout=60))
